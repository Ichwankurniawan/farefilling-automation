...
| 13 | Accompanied Travel | Coded | `has_mapping=False`, verified via template: no category-specific columns exist at all. **UPDATED**: real condition text (when non-empty/non-boilerplate) now captured into a JSON-only `Note` field -- see bug/decision #32 |
...
| 18 | Ticket Endorsement | Coded | `has_mapping=False` -- confirmed since the very start of this project (rich-looking text, but DataMapping explicitly says no field-level mapping exists). **UPDATED**: this is the one category of the 12 confirmed to have genuinely rich, non-boilerplate real text on EVERY real sample checked, not just hypothetically -- now captured into a JSON-only `Note` field every run (a real, recurring AI call, not free) -- see bug/decision #32 |
...
| 20, 21, 22, 26, 27, 28, 29, 33 | Tour Conductor / Agent / Other Discounts / Groups / Tours / Visit Another Country / Deposits / Voluntary Refunds | Coded | `has_mapping=False` -- DataMapping explicitly says "no category-specific mapping defined" for every one of these. Template DOES have real category-specific columns for several (e.g. CAT20/21 mirror CAT19's Passenger Type/Age/PSGR Occur shape), but no data source exists to populate them (confirmed -- the source pricebook tab these would need doesn't exist in any real file seen so far, same finding as the CAT05/06/07 ADV Res block, see the CLAUDE.md history around that check). **UPDATED**: real condition text (when non-empty/non-boilerplate) now captured into a JSON-only `Note` field -- see bug/decision #32. CAT20/21/22/26/27/28/29 all read as the boilerplate no-restriction phrase on every real sample checked so far (zero-cost no-op); CAT33's real text is a bare pointer label (`"FARE FAMILY FEE CONDITIONS"`), which does trigger a real AI call, correctly recognized and reported as a pointer rather than hallucinated into real content |
| 23 | Miscellaneous Provisions | Coded | `has_mapping=False`. "Override Dates" explicitly marked CONSTANT "Not needed -- the format uses calendar dates". **Column layout quirk**: this category's block is shifted one position left vs every other category -- LOC1 is missing entirely (Zone1 sits first), AltGenTariff/AltGenRule/OW-RT are at O/P/M instead of the usual P/Q/N. **UPDATED**: real condition text (genuinely non-boilerplate on every real sample checked -- "add-on construction"/"differential" restrictions) now captured into a JSON-only `Note` field -- see bug/decision #32 |
| 31 | Voluntary Changes | Coded | `has_mapping=False` for category-specific fields (pointer to unseen "FARE FAMILY FEE CONDITIONS" source, same pattern as CAT16). AltGenRule works automatically via the standard per-category IPRG mechanism (confirmed: Fare Rules "IPRG 0011" -> AltGenRule="0011"). DataMapping also describes a Type-2-only override pattern (same shape as CAT14's) -- out of scope for Type 1. `ai_spec_file` (`cat31_spec.yaml`) is a genuinely separate, still-unused slot -- reserved in case a category-specific spec is ever needed later, not read by the new generic `Note` fallback below (which uses its own shared `no_mapping_note_spec.yaml` regardless of what `ai_spec_file` is set to). **UPDATED**: real condition text (a bare `"FARE FAMILY FEE CONDITIONS"` pointer label on the one real sample checked) now captured into a JSON-only `Note` field -- correctly recognized and reported as a pointer, not hallucinated -- see bug/decision #32 |
...
32. **Every `has_mapping=False` category (all 12 -- CAT13, 18, 20-23, 26-29, 31, 33) now captures real condition text into a JSON-only `Note` field, instead of `resolve()` discarding it unconditionally before ever reading it.** Direct follow-on from bug #31 -- CAT16 proved the "JSON-only Note for a category with zero Excel destination" pattern was worth generalizing rather than repeating per-category (which is what CAT09/CAT16 each did independently). New shared `_resolve_no_mapping()` in `base.py` (replacing the old unconditional-blank-entry branch of `resolve()`): reads the category's own condition text (`pricebook_data.get_fare_rules_row(self.category[3:])`, defensively `hasattr`-guarded), and only calls AI when it's genuinely neither empty nor the standard no-restriction phrase (`_is_none_condition()` -- same guard as bug #30 and its follow-ups) -- zero-cost no-op otherwise. One new shared spec, `ai_specs/no_mapping_note_spec.yaml`, used by all 12 categories -- no per-category yaml/`output_fields` declaration needed, unlike CAT09/CAT16's earlier hand-rolled versions. `Note` is never written to Excel (there's nowhere to write it, confirmed for all 12 via the DataMapping doc), only visible in the JSON output.

    Verified two ways. A fast mocked sweep across all 12 categories x 3 input scenarios (empty / NONE-phrase / genuine content) confirmed the gating is uniformly correct -- free on the first two, AI called only on the third, every time. Then real, unmocked verification against HKF1: **CAT20** (boilerplate real text) stayed free at 0.00s, confirming the zero-cost claim holds on real data, not just synthetic; **CAT18** and **CAT23** (both confirmed genuinely rich, non-boilerplate real text on every sample checked -- not hypothetical) correctly produced real, useful summaries (e.g. CAT18: *"Change/refund/no-show fees apply for U/P/L/M/H/W/Q/N fare types; for D/R fare types..."*); **CAT31** and **CAT33** (both bare `"FARE FAMILY FEE CONDITIONS"` pointer-label text) correctly reported that the text is a pointer to an unseen external document rather than hallucinating fee amounts or restrictions that aren't actually stated -- confirms the spec's explicit "say so plainly rather than inventing content" instruction works in practice, not just on paper.

    **Worth knowing, not a limitation**: CAT18 is the one category of the 12 where this is a real, recurring cost (~15-40s, real tokens, every run of a rule with this text shape) rather than one-time future-proofing -- its real content is genuinely rich on every sample seen, unlike the other 11 which are boilerplate/pointer text in every real sample checked so far.

    New `tests/test_no_mapping_ai_fallback.py` locks in the empty/both-NONE-phrases-free and real-content-reaches-AI cases (mocked, not a real call), plus the defensive `hasattr` guard's fallback behavior. Full suite: 55/55 passing in 0.5s.
