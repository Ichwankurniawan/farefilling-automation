...
class Cat05Resolver(CategoryResolver):
    category = "CAT05"
    has_mapping = True
    ai_spec_file = "cat05_spec.yaml"
    output_fields = [
        "LOC1", "Zone1", "LOC2", "Zone2", "FareClassFamily",
        "ADVDuration", "ADVUnit",
        # Time, NBR, Day, TSI, LOC (ADV Res block), Confirmed Sectors, and
        # both Ticketing blocks have no documented column source yet --
        # intentionally left out of output_fields rather than guessed.
    ]

    def _resolve_impl(self, rule_id, pricebook_data, sheet_type):
        row = pricebook_data.get_fare_rules_row("05")
        if row is None:
            return self._bundle(rule_id, sheet_type, "UNRECOGNIZED",
                                 [self._flagged_entry("No CAT05 row found in Fare Rules")])
    ...
    def _map_row(self, r, pricebook_data):
        ...
        entry["ADVDuration"] = r.get("ADV PURCHASE")
        entry["ADVUnit"] = r.get("ADV PURCHASE UNIT")

        # Overrides common_fields.py's generic OW/RT fallback -- CAT05 has
