...
class Cat06Resolver(CategoryResolver):
    category = "CAT06"
    has_mapping = True
    ai_spec_file = "cat06_spec.yaml"
    output_fields = [
        "LOC1", "Zone1", "LOC2", "Zone2", "FareClassFamily",
        "Duration", "Unit", "MeasuredFromTSI", "ReturnTravelFromTSI",
    ...
    def _resolve_impl(self, rule_id, pricebook_data, sheet_type):
        row = pricebook_data.get_fare_rules_row("06")
        if row is None:
            return self._bundle(rule_id, sheet_type, "UNRECOGNIZED",
                                 [self._flagged_entry("No CAT06 row found in Fare Rules")])
    ...
    def _map_row(self, r, pricebook_data):
        ...
        entry["Duration"] = r.get("MIN STAY")
        entry["Unit"] = r.get("MIN STAY UNIT")
        entry["MeasuredFromTSI"] = "01"           # CONSTANT
        entry["ReturnTravelFromTSI"] = "05"       # CONSTANT

        # Overrides common_fields.py's generic OW/RT fallback -- CAT06 has
