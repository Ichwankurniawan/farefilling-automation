...
class Cat11Resolver(CategoryResolver):
    category = "CAT11"
    has_mapping = True
    ai_spec_file = "cat11_spec.yaml"
    output_fields = [
        "LOC1", "Zone1", "LOC2", "Zone2", "FareClassFamily",
        "DayRange", "Date1", "Date2",
        # "CAT (restrict to another category)" is MANUAL -- loader decides,
        # not extractable from the pricebook.
    ]

    def _resolve_impl(self, rule_id, pricebook_data, sheet_type):
        row = pricebook_data.get_fare_rules_row("11")
        if row is None:
            return self._bundle(rule_id, sheet_type, "UNRECOGNIZED",
                                 [self._flagged_entry("No CAT11 row found in Fare Rules")])
        ...
        rows = pricebook_data.get_tab(target_tab) or pricebook_data.get_tab("CAT11-Blackouts")
        if not rows:
    ...
    def _map_row(self, r, pricebook_data):
        ...
        entry["Date1"] = self._lookup(r, "PERIOD START")
        entry["Date2"] = self._lookup(r, "PERIOD END")
        # Day/Range = "Range" when both Date1 and Date2 are present (a
        # date range) -- confirmed by the user. No example yet of a
        # single-day case to confirm what value that would use.
        if entry["Date1"] and entry["Date2"]:
            entry["DayRange"] = "Range"

        # Overrides common_fields.py's generic OW/RT fallback -- CAT11 has
